...
def get_thread_messages(
    client: WebClient, view_metadata: dict, include_bot_msg: bool = False
) -> list:
    """
    ...
    if not view_metadata.get("thread_ts"):
        ...
        try:
            response = client.conversations_replies(
                channel=view_metadata["channel_id"], ts=view_metadata["thread_ts"]
            )
            messages = response["messages"]
        except SlackApiError as e:
    ...
    messages = (
        messages
        if include_bot_msg
        else [msg for msg in messages if "bot_id" not in msg and "app_id" not in msg]
    )
    ...
    try:
        client.views_open(
            ...
            view={
                ...
                "title": {"type": "plain_text", "text": "Summarise the thread"},
                "submit": {"type": "plain_text", "text": "Summarise"},
...
def open_summary_review_modal(ack, body, client, view, logger):
    ...
    ack(
        ...
        view={
            ...
            "blocks": block_builder.build_loading_blocks(
                "Generating summary of the thread via OpenAI..."
            ),
        },
    )
...
def regenerate_summary(ack, body, client, logger):
    ...
    model_temperature = view_metadata.get(
        "model_temperature"
...
def update_summary_review_modal(ack, body, client, view, logger):
    ...
    for section in constants.SUMMARY_SECTIONS:
        updated_thread_summary[section] = (
            user_input.get(section).get("input_value").get("value")
        )
...
def upload_file_modal(ack, body, client, view, logger):
    ...
    ack(
        ...
        view={
            "type": "modal",
            "close": {"type": "plain_text", "text": "Cancel", "emoji": True},
            "title": {"type": "plain_text", "text": "Upload", "emoji": True},
            "blocks": block_builder.build_loading_blocks(
                "Uploading content to confluence..."
            ),
        },
    )
    ...
    messages = get_thread_messages(client, view_metadata, False)
    users = [
        get_user_info(client, user_id) for user_id in messages[0].get("reply_users", [])
    ]
    message_link = get_permalink(
        client, view_metadata.get("channel_id"), messages[0].get("ts")
    )
    markdown_content = confluence_helper.convert_summary_to_markdown(
        thread_summary, messages, users, message_link
    )
    ...
    client.chat_postMessage(
        ...
        blocks=block_builder.build_final_message_blocks(
            {
                "title": title,
                "link": f"{constants.CONFLUENCE_BASE_URL}{response_text.get('_links').get('webui')}",
            }
        ),
    )
