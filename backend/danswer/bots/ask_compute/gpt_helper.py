...
logger = setup_logger("ask_compute_bot.gpt_helper", constants.LOG_LEVEL)

@retry(wait=wait_random_exponential(multiplier=1, max=40), stop=stop_after_attempt(3))
def chat_completion_request(endpoint, api_key, model, messages, temperature=None, functions=None, function_call=None):
    headers = {
...
def get_thread_summary_prompt(messages: list) -> list:
    ...
    for msg in messages:
        prompt.append({"role": "user", "content": f"(<@{msg.get('user')}>): ${msg.get('text')}"})
    # Append prompt to summarise the thread content into a technical document
    prompt.append({"role": "user", "content": "Please follow the rules outlined in the system message to generate the summary document."})
    return prompt
