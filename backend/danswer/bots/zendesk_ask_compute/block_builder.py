    ...
    return convert_blocks_to_dict(
        [
            SectionBlock(
                text=TextObject(
                    type="mrkdwn",
                    text=f"*Hi <@{block_data.get('user', {}).get('id', None)}>! Please configure:*",
                )
            ),
            DividerBlock(),
            SectionBlock(
                ...
                accessory=StaticSelectElement(
                    ...
                    options=[
                        Option(
                            text=TextObject(type="plain_text", text="GPT-4"),
                            value="gpt-4",
                        ),
                        Option(
                            text=TextObject(type="plain_text", text="GPT-4-32K"),
                            value="gpt-4-32k",
                        ),
                        Option(
                            text=TextObject(type="plain_text", text="GPT-3.5"),
                            value="gpt-3.5-turbo",
                        ),
                    ],
                    initial_option=Option(
                        text=TextObject(type="plain_text", text="GPT-4"), value="gpt-4"
                    ),
                ),
            ),
            InputBlock(
            ...
            SectionBlock(
                text=TextObject(
                    type="mrkdwn", text=":man-getting-massage: *Place holder*\n  TBD"
                ),
                accessory=StaticSelectElement(
                    ...
                    options=[
                        Option(
                            value="option-1",
                            text=TextObject(type="plain_text", text="Option 1"),
                        )
                    ],
                ),
            ),
...
def build_summary_review_blocks(block_data: dict) -> list[dict]:
        ...
        return convert_blocks_to_dict(
            [
                SectionBlock(
                    text=TextObject(
                        type="mrkdwn",
                        text="The thread does not include an issue or a question.",
                    )
                ),
            ]
        )
    section_blocks = [
        SectionBlock(
            text=TextObject(
                type="mrkdwn",
                text=f"*{section.replace('_', ' ').capitalize()}*\n{block_data.get(section, None)}",
            )
        )
    ...
    blocks = convert_blocks_to_dict(
        [
            SectionBlock(
                text=TextObject(
                    type="mrkdwn",
                    text="Please review generated summary before submitting to a knowledge base.",
                )
            ),
            DividerBlock(),
        ]
